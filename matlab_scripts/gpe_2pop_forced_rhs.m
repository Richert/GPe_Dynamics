function [du, I_p, I_a] = gpe_2pop_forced_rhs(u,s_pp,s_ap,s_pa,s_aa,I_ext_p,I_ext_a,...
    eta_p,eta_a,N_p,N_a,k_pp,k_pa,k_ap,k_aa,alpha,t_on,t_off,tau_p,tau_a)
%GPE_2POP_FORCED_RHS Dynamic equations of a two-population gpe qif network with periodic forcing and sparse coupling.
...
I_p = u(N_p+1:2*N_p);
y_p = u(2*N_p+1:3*N_p);
r_pp_1 = u(3*N_p+1:4*N_p);
r_pp_2 = u(4*N_p+1:5*N_p);
r_pa_1 = u(5*N_p+1:6*N_p);
r_pa_2 = u(6*N_p+1:m_p);

% extract state variables at gpe-a
v_a = u(m_p+1:m_p+N_a);
I_a = u(m_p+N_a+1:m_p+2*N_a);
y_a = u(m_p+2*N_a+1:m_p+3*N_a);
r_ap_1 = u(m_p+3*N_a+1:m_p+4*N_a);
r_ap_2 = u(m_p+4*N_a+1:m_p+5*N_a);
r_aa_1 = u(m_p+5*N_a+1:m_p+6*N_a);
r_aa_2 = u(m_p+6*N_a+1:m_p+m_a);
