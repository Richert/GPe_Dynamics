...
% cells (both QIF populations) with sparse synaptic connectivity.
%
...
k_pa = k*k_gp*k_i/k_p;
k_aa = k*k_gp/(k_i*k_p);
C_pp = get_sparse_mat(N_p, N_p, p, 0);
C_ap = get_sparse_mat(N_p, N_a, p, 0);
C_pa = get_sparse_mat(N_a, N_p, p, 0);
C_aa = get_sparse_mat(N_a, N_a, p, 0);
...
u = zeros(1,N);
u(1:N_p) = v_mac_init;
u(7*N_p+1:7*N_p+N_a) = v_mac_init;
u(m_p+m_a+1) = 1.0;

% GPe-p
wait_p = zeros(1,N_p);
spike_n_p = sparse(zeros(1,N_p));
spike_t_p = wait_p;
...
for t = 1:T
    ...
    [du, Ip, Ia] = gpe_2pop_forced_rhs(u,s_pp,s_ap,s_pa,s_aa,I_p(1,t),I_a(1,t),...
    ...
    u(m_p+1:m_p+N_a) = u(m_p+1:m_p+N_a) + dt .* mask_a .* du(m_p+1:m_p+N_a);
    u(m_p+N_a+1:end) = u(m_p+N_a+1:end) + dt .* du(m_p+N_a+1:end);

    % spiking mechanism
    [vp, spike_t_p, wait_p] = spiking_mechanism(u(1:N_p), v_th_p, dt, wait_p, ...
        spike_t_p, tau_p, eta_p - tau_p*Ip + I_p(1, t));
    [va, spike_t_a, wait_a] = spiking_mechanism(u(m_p+1:m_p+N_a), v_th_a, dt, ...
    ...
    rp_rec(t_buffer+1) = mean(rp);
    vp_rec(t_buffer+1) = mean(vp(wait_p==0));
    ra_rec(t_buffer+1) = mean(ra);
